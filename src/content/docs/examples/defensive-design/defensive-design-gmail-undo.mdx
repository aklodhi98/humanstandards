...
```typescript
// Get component guidance for a destructive action
...
{
  defensive_design: {
    recommendation: 'Implement undo with delay instead of confirmation',
    pattern: 'delayed-execution-with-undo',
    timing: '5-10 seconds for low-risk, 10-30 seconds for high-risk',
    rationale: 'Less disruptive than confirmation, catches immediate regret',
    reference: '/decision-making-errors/defensive-design.md'
  },
  feedback: {
    required: [
      'Clear visual indicator of pending action',
      'Prominent undo button',
      'Progress indicator showing remaining time',
      'Immediate feedback when undone'
    ],
    reference: '/interaction-patterns/feedback.md'
  },
  accessibility: {
    requirements: [
      'Announce undo option to screen readers',
      'Keyboard shortcut for undo (Ctrl+Z)',
      'Sufficient time to perceive and act (min 5 seconds)'
    ],
