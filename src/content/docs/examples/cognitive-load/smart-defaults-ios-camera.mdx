...
<CardGrid>
  ...
  <Card title="Decision-Making Errors" icon="warning">
    [Eliminating choice overload](/decision-making-errors/cognitive-biases/) through intelligent defaults.
  </Card>
  ...
  <Card title="User Control & Freedom" icon="approve-check">
    [Balancing automation with control](/interaction-patterns/user-control/) - defaults for novices, options for experts.
  </Card>
</CardGrid>
...
```typescript
// Get component guidance for settings interface
const guidance = await mcp.callTool('get_component_guidance', {
  component: 'settings',
  context: {
    user_expertise: 'mixed',
    options_count: 20
  }
});
...
{
  cognitive_load: {
    assessment: '20 options create high cognitive load and choice overload',
    recommendation: 'Use smart defaults + progressive disclosure pattern',
    implementation: [
      'Set intelligent defaults for all 20 options',
      'Show only 3-5 most common settings initially',
  ...
  smart_defaults: {
    ...
    examples: [
      'Use device capabilities (detect dark mode, locale)',
      'Learn from user behavior (remember last used settings)',
      'Apply sensible defaults (privacy = strict, quality = high)'
    ],
    reference: '/decision-making-errors/cognitive-biases.md'
  },
  progressive_disclosure: {
    pattern: 'basic-advanced-expert',
    basic: 'Smart defaults, zero configuration needed',
    advanced: '"Show more options" reveals 5-7 common tweaks',
    expert: '"Advanced settings" unlocks all 20 options',
    reference: '/interaction-patterns/progressive-disclosure.md'
  }
}
```
...
- [Progressive Disclosure - TurboTax](/examples/progressive-disclosure-turbotax/) - Hide complexity until needed
- [Error Prevention - Grammarly](/examples/error-prevention-grammarly/) - Intelligent assistance reduces mistakes
- [Defensive Design - Gmail Undo](/examples/defensive-design-gmail-undo/) - Forgiveness over permission
